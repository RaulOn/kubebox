...
class Kubebox extends EventEmitter {
  ...
  constructor(screen, server) {
    ...
    const kdashboard = new KDashboard({ screen, navbar, status, client });

    const page = navbar.add({ title: 'Pods', listener: dashboard }, { select: true });
    const kservice = navbar.add({ title: 'Knative', listener: kdashboard });
    ...
    screen.key(['n'], () => {
      namespaces.prompt(screen, client, { current_namespace })
        .then(namespace => {
          ...
          kdashboard.reset(kservice);
          dashboard.reset(kservice);
          // switch dashboard to new namespace
          current_namespace = namespace;
          console.log(`Switching to namespace ${current_namespace}`);
          screen.render();
          kdashboard.run(current_namespace);
          dashboard.run(current_namespace);
          return;
        })
    ...
    function connect(login, options = {}) {
      ...
      cancellations.add('connect', cancel(cancellation));
      return until(promise
        .then(() => {
          // try calling the discovery API
          const { promise, cancellation } = client.paths().get({ cancellable: true });
          cancellations.add('connect', cancel(cancellation));
          return promise.then(response => client._paths = JSON.parse(response.body.toString('utf8')).paths)
            .catch(error => log(`{red-fg}Error calling discovery API: ${error.message}{/red-fg}`));
        })
        .then(() => current_namespace || namespaces
          ...
          .then(namespace => namespace
            ? current_namespace = namespace
            : Promise.reject()))
        .then(namespace => {Promise.all([kdashboard.run(namespace), dashboard.run(namespace)])}))
    ...
    function logging(options = { closable: false }) {
      ...
      return promise
        .then(login => {
          ...
          dashboard.reset(kservice);
          updateSessionAfterLogin(login);
          return connect(login, Object.assign({}, options, { closable: false }));
        });
    }
    ...
    function authenticate(login) {
      ...
      if (isNotEmpty(login.token) && isEmpty(login.password)) {
        ...
        const { promise: p, cancellation } = client.oauth_authorize_web(login).get({ cancellable: true })
        cancellations.add('connect', cancel(cancellation));
        promise = p.then(response => {
        ...
        const { promise: p, cancellation } = client.oauth_authorize(login).get({ cancellable: true })
        cancellations.add('connect', cancel(cancellation));
        promise = p.then(response => response.headers.location.match(/access_token=([^&]+)/)[1]);
      }

      return promise.then(token => {
        // test it authenticates ok
        const { promise: p, cancellation } = client.user(token).get({ cancellable: true });
        cancellations.add('connect', cancel(cancellation));
        return p.then(response => {
          // then set the authorization heade
