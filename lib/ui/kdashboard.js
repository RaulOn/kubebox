...
class KDashboard extends EventEmitter {
  ...
  constructor({ screen, navbar, status, client }) {
    ...
    pods_table.key(['r'], () => {
      ...
      until(promise)
        ...
        .then(() => {
          ...
          const { promise, cancellation } = client.kservices(namespace).fieldSelector(`metadata.name=${name}`).watch(pod.metadata.resourceVersion).get({ generator: function* () {
            console.log("Watching kservices")
            let change;
            while (change = yield) {
    ...
    pods_table.on('select', (item, i) => {
      ...
      cancellations.run('kdashboard.pod');
    ...
    function selectContainer(pod, container) {
      ...
      const logger = function* (sinceTime) {
        ...
        cancellations.add('kdashboard.pod.logs', () => log.cancel());
        try {
          while (data = yield) {
        ...
        pause(1000)
          ...
          .then(response => {
            ...
            const { promise, cancellation } = client.pod(namespace, pod.metadata.name).log({ container: container.name, sinceTime: timestamp }).get({ generator: timestamp
              ...
              : logger });
            cancellations.add('kdashboard.pod.logs', cancellation);
      ...
      cancellations.add('kdashboard.pod.logs', logs.cancellation);
      until(logs.promise)
        .do(pod_log, pod_log.setLabel)
        .spin(s => `${s} Logs {grey-fg}[${container.name}]{/grey-fg}`)
        .cancel(c => cancellations.add('kdashboard.pod.logs', c))
        .catch(error => {
      ...
      let cancelled = false;
      let cancellation = () => {};
      cancellations.add('kdashboard.pod.stats', () => {
        cancellation();
        cancelled = true;
      });
    }
    ...
    function updatePodsTable() {
      ...
      pods_table.setData((pods_list.items || []).reduce((rows, pod) => {
        ...
        rows.push(row);
        return rows;
      }, [['NAME', 'URL', 'LATESTCREATED', 'LATESTREADY', 'READY', 'AGE']]));
    ...
    this.reset = function (page) {
      ...
      pods_table.setLabel('Kservice reset');
      pods_table.setData([]);
      if (pods_table_text) {
    ...
    this.run = function (namespace) {
      console.log('Kdashboard load ' + namespace)
      current_namespace = namespace;
      let listPodsError;
      let { promise, cancellation } = client.kservices(namespace).get({ cancellable: true });
      cancellations.add('kdashboard.pods', cancellation);
      promise = until(promise)
        .do(pods_table, pods_table.setLabel)
        .spin(s => `${s} Pods {grey-fg}[${namespace}]{/grey-fg}`)
        .cancel(c => cancellations.add('kdashboard.pods', c))
        .done(_ => `Pods {grey-fg}[${namespace}]{/grey-fg}`)
        .then(response => {
          console.debug("response: " + response.body.toString('utf8'))
      ...
      promise
        .then(() => {
          console.debug(`Watching for kservice changes in namespace ${namespace} ...`);
          screen.render();
          const id = setInterval(refreshPodAges, 1000);
          cancellations.add('dashboard.refreshPodAges', () => clearInterval(id));
          const { promise, cancellation } = client.kservices(namespace)
    ...
    function* watchPodChanges(namespace) {
      ...
      cancellations.run('kdashboard.refreshPodAges');
